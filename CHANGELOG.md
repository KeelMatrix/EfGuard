...
- Console reports render the reason, affected state, risk dimensions, provider evidence, remediation guidance, and uncertainty; versioned JSON reports retain the same stable diagnostic contract and exit codes `0` (no configured blocking or unverified diagnostic), `1` (a blocking or unverified diagnostic), and `2` (analysis could not complete trustworthily because of configuration, extraction, build, provider, baseline, or internal error).
...
- Documentation now has one canonical pinned release install example, and changelog validation rejects duplicate, unpinned, divergent, or multiline-competing examples.
- Added an explicit MSB4166 node-isolation matrix regression test for worker builds, target-framework probes, local validation scripts, and CI/release workflows; validation now reports the exact package artifact when a locked output prevents cleanup.
...
- `--context` now scopes migration discovery to the migrations EF Core attributes to the selected `DbContext`: extraction reads EF Core's own migrations metadata and never falls back to every `Migration` subclass in the migrations assembly, so migrations that belong to another context are no longer analyzed for the selected context. A context without attributed migrations reports zero operations when its migrations assembly has no unclaimed migration classes; if no migrations are attributed to the selected context while that assembly contains migration classes that no `[DbContext]` attribute claims, extraction fails closed with an actionable diagnostic instead of silently producing a clean scan.
