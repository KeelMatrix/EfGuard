...
The supported dependency graph includes the standard design-time package `Microsoft.EntityFrameworkCore.Design`, which the `dotnet ef` tooling requires, including when only a separate `--startup-project` references it. Extraction enumerates metadata per copied assembly, so a design-time, build, or Roslyn assembly that cannot be loaded inside the worker is skipped with a recorded note instead of failing the scan. Fail-closed behavior is preserved where it matters: when the assembly that declares the selected `DbContext` or its migrations cannot be enumerated at all, extraction stops with the assembly name, the underlying exception, and a corrective next step.
...
- **A copied dependency assembly cannot be enumerated:** the worker copies the selected project's dependency graph and reads metadata per assembly, so a design-time, build, or Roslyn assembly that cannot be loaded in the worker process is skipped with a recorded note instead of failing the whole scan. If the assembly that declares the selected `DbContext` or its migrations cannot be enumerated at all, extraction fails closed with exit code `2` and names the assembly and the underlying exception. Rebuild the project (`dotnet build`) and run EfGuard again; if the failure persists, resolve the reported exception, which usually means the assembly needs a missing dependency or a runtime it was not built for.
- **A migration class is reported as unattributed:** EF Core applies and lists a migration only when the migration carries `[DbContext(typeof(...))]`; EF Core itself writes that attribute into the migration designer files it generates. EfGuard follows the same rule, so a migration class without a matching `[DbContext]` attribute is never attributed to the selected context and instead produces an actionable extraction failure with exit code `2`. Add the attribute, or run the scan for the context that owns those migrations.
...
- **An EF metadata assembly is only partially loadable:** a partial type load in the assembly declaring the selected `DbContext`, or in the migrations assembly EF Core attributes to that context, fails closed with exit code `2` and names the assembly and loader exception. Partial loads in unrelated design-time or other peripheral assemblies are tolerated; EfGuard emits a bounded `EfGuard note:` diagnostic on standard error for each skipped assembly.
